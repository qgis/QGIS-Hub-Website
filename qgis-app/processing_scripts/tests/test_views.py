...
class TestFormValidation(SetUpTest, TestCase):
    ...
    def test_form_with_valid_decorator_script(self):
        ...
        uploaded_script = SimpleUploadedFile(
            self.alg_decorator_script_content.name,
            self.alg_decorator_script_content.read(),
        )
        data = {
    ...
    def test_form_with_pyqt_version_pyqt5(self):
        """Test form validation with PyQt5 version specified"""
        uploaded_thumbnail = SimpleUploadedFile(
            self.thumbnail_content.name, self.thumbnail_content.read()
        )
        uploaded_script = SimpleUploadedFile(
        ...
        data = {
            "name": "script with pyqt5",
            "description": "Test upload with PyQt5 version",
            "dependencies": "QuickOSM",
            "pyqt_versions": [self.pyqt5.id],
        }
        file_data = {"thumbnail_image": uploaded_thumbnail, "file": uploaded_script}
        form = UploadForm(data, file_data)
        self.assertTrue(form.is_valid())

    def test_form_with_pyqt_version_pyqt6(self):
        """Test form validation with PyQt6 version specified"""
        uploaded_thumbnail = SimpleUploadedFile(
            self.thumbnail_content.name, self.thumbnail_content.read()
        )
        uploaded_script = SimpleUploadedFile(
        ...
        data = {
            "name": "script with pyqt6",
            "description": "Test upload with PyQt6 version",
            "dependencies": "QuickOSM",
    ...
    def test_form_with_multiple_pyqt_versions(self):
        """Test form validation with multiple PyQt versions"""
        uploaded_thumbnail = SimpleUploadedFile(
            self.thumbnail_content.name, self.thumbnail_content.read()
        )
        uploaded_script = SimpleUploadedFile(
        ...
        data = {
            "name": "script with multiple pyqt versions",
            "description": "Test upload with both PyQt versions",
            "dependencies": "QuickOSM",
...
class TestUpdateScript(SetUpTest, TestCase):
    ...
    def test_processing_script_default_pyqt_version(self):
        """Test that ProcessingScript has no default pyqt_versions"""
        self.assertEqual(self.processing_script.pyqt_versions.count(), 0)

    def test_update_script(self):
        self.client.login(username="creator", password="password")
        url = reverse(
            "processing_script_update", kwargs={"pk": self.processing_script.id}
        )
        ...
        self.assertRedirects(
            response,
            reverse(
                "processing_script_detail", kwargs={"pk": self.processing_script.id}
            ),
        )
...
class TestReviewProcessingScript(SetUpTest, TestCase):
    ...
    def test_approve_processing_script(self):
        ...
        url = reverse(
            "processing_script_review", kwargs={"pk": self.processing_script_object.id}
        )
        ...
        url = reverse(
            "processing_script_detail", kwargs={"pk": self.processing_script_object.id}
        )
