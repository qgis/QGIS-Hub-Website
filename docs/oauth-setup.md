...
- **Sites** — move your site (e.g. `hub.qgis.org`) from "Available" to "Chosen"
...
2. Fill in:
   - **Application name**: QGIS Hub (or any label)
   - **Homepage URL**: `https://hub.qgis.org`
   - **Authorization callback URL**: `https://hub.qgis.org/accounts/github/login/callback/`
...
4. Add to **Authorized redirect URIs**:
   ```
   https://hub.qgis.org/accounts/google/login/callback/
   ```
...
2. Fill in:
   - **Name**: QGIS Hub
   - **Redirect URI**: `https://hub.qgis.org/accounts/gitlab/login/callback/`
...
2. Fill in:
   - **Name**: QGIS Hub
   - **Main Application URL**: `https://hub.qgis.org`
   - **Callback URL**: `https://hub.qgis.org/accounts/openstreetmap/login/callback/`
...
2. Fill in:
   ...
   - **Redirect URI**: Web — `https://hub.qgis.org/accounts/microsoft/login/callback/`
...
1. **Create an App ID**:
   ...
   - Note the **Bundle ID** (e.g. `org.qgis.hub`)

2. **Create a Services ID** (this is the OAuth client):
   - Go to **Identifiers → +** → **Services IDs**
   - Description: QGIS Hub
   - Identifier: e.g. `org.qgis.hub.web` (must be unique, different from Bundle ID)
   - Enable **Sign In with Apple → Configure**:
     - Primary App ID: select the App ID from step 1
     - Domains: `hub.qgis.org`
     - Return URLs: `https://hub.qgis.org/accounts/apple/login/callback/`
...
4. In Django Admin:
   - Provider: `Apple`
   - Client ID: the **Services ID** identifier (e.g. `org.qgis.hub.web`)
...
3. After creation, go to the **Auth** tab:
   ...
   - Add to **Authorized redirect URLs for your app**:
     ```
     https://hub.qgis.org/accounts/linkedin_oauth2/login/callback/
     ```
...
3. Send `/setdomain` to BotFather, select your bot, and enter your domain: `hub.qgis.org`.
4. In Django Admin:
   - Provider: `Telegram`
   - Client ID: your **bot username** (without `@`, e.g. `QGISHubBot`)
   - Secret key: the **bot token** (e.g. `123456789:ABC-...`)

> The Telegram provider works via a login widget that calls back to  
> `https://hub.qgis.org/accounts/telegram/login/callback/`.  
...
Prepend your domain, e.g. `https://hub.qgis.org/accounts/github/login/callback/`.
